...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        if len(hidden_dims) > 1:
          for i, hidden_num in enumerate(hidden_dims[1:]):
            layer_idx = i + 1
            layer = layer_idx + 1
            W = init_weights(hidden_dims[i], hidden_num)
            b = np.zeros(hidden_num)

            self.params['W%d' % layer] = W
            self.params['b%d' % layer] = b

            if use_batchnorm:
              self.params['gamma%d' % layer] = np.ones(hidden_num)
              self.params['beta%d' % layer] = np.zeros(hidden_num)
        ...
        self.params['W%d' % last_layer_idx] = \
          init_weights(last_hidden_dim, num_classes)
        self.params['b%d' % last_layer_idx] = np.zeros(num_classes)
    ...
    def loss(self, X, y=None):
        ...
        for layer_idx in range(self.num_layers - 1):
          ...
          if self.use_batchnorm:
            gamma, beta = \
              self.params['gamma%d' % layer], self.params['beta%d' % layer]

            batch_norm_out, bn_cache = \
              batchnorm_forward(out_x, gamma, beta, self.bn_params[layer_idx])
            cache['bn'].append(bn_cache)
        ...
        for layer_idx in reversed(range(self.num_layers - 1)):
          ...
          if self.use_batchnorm:
            bn_cache = cache['bn'][layer_idx]
            dbn, dgamma, dbeta = batchnorm_backward(dh, bn_cache)
